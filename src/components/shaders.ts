...
export const fragmentShader = `
  uniform float uTransition; // Controls progress (0.0 = solid, 1.0 = fully revealed)
  uniform vec2 uResolution;
  uniform float uTime;
  varying vec2 vUv;

  // Customizable border color uniform (Cherry Red)
  ...
  void main() {
    // 1. PIXELATION FILTER: Snap continuous UV coordinates onto discrete pixel blocks
    float pixelSize = 10.0; 
    vec2 grid = uResolution / pixelSize;
    vec2 pixelatedUv = floor(vUv * grid) / grid;

    // 2. Correct aspect ratios to prevent widescreen noise distortion
    float aspect = uResolution.x / uResolution.y;
    vec2 correctedUv = (pixelatedUv - 0.5) * vec2(aspect, 1.0) + 0.5;

    // Calculate maximum possible distance to any corner
    ...
    // Radial gradient thresholding: center opens up first and noise moves outward to edges
    float radialGradient = normalizedDistance * 12.5 + (1.0 - uTransition) * 2.0 - 15.0 * uTransition;
    
    // Store unclamped raw strength
    float rawStrength = strength + radialGradient;

    // Clamp value from 0 to 1 for opacity boundary
    strength = clamp(rawStrength, 0.0, 1.0);

    // Volumetric edge detection for glowing Cherry Red border
    float edge = smoothstep(0.0, 0.7, rawStrength) * smoothstep(2.5, 0.7, rawStrength);
    edge *= min(uTransition * 5.0, 1.0);

    // Deriving rich glowing Cherry Red colors
    vec3 deepMidnightColor = uBorderColor * 0.02; // Dark crimson tone
    vec3 richGlowingColor = uBorderColor * 1.8;   // Rich Cherry Red glowing tone
    ...
    // Final alpha: 1.0 = solid screen veil, 0.0 = transparent hole revealing homepage simultaneously
